package com.test.framework.util.time;

import com.test.framework.FrameworkConstant;
...
public class DateTimeUtil {
    ...
    public static String getDateTimeFromDateType(Date date) {
        return DateTimeFormatterUtil.convertDatetimeToString(convertDateToLocalDateTime(date));
    }
    ...
    public static String getCurrentDatetimeString() {
        return LocalDateTime.now().format(DateTimeFormatterUtil.DATETIME_FORMATTER);
    }
    ...
    public static String getCurrentDateString() {
        return LocalDate.now().format(DateTimeFormatterUtil.DATE_FORMATTER);
    }
    ...
    public static String getCurrentTimeString() {
        return LocalDateTime.now().format(DateTimeFormatterUtil.TIME_FORMATTER);
    }

    // todo: 需要考虑发生异常是在底层处理还是在调用处处理
    public static Date getDateFromString(String datetime) {
        try {
            LocalDateTime localDateTime = DateTimeFormatterUtil.convertStringToDatetime(datetime);
            ZoneId zone = ZoneId.systemDefault();
    ...
    public static boolean isAfterPresentTime(String str) {
        try {
            LocalDateTime dateTime = DateTimeFormatterUtil.convertStringToDatetime(str);
    ...
    public static boolean isAfterLatterTime(String t1, String t2) throws Exception {
        if (DateTimeFormatterUtil.isValidTime(t1) && DateTimeFormatterUtil.isValidTime(t2)) {
            return isAfterLatterDateTime(t1, t2, DateTimeFormatterUtil.TIME_FORMAT_PATTERN);
        } else if (DateTimeFormatterUtil.isValidDate(t1) && DateTimeFormatterUtil.isValidDate(t2)) {
            return isAfterLatterDateTime(t1, t2, DateTimeFormatterUtil.DATE_FORMAT_PATTERN);
        } else if (DateTimeFormatterUtil.isValidDateTime(t1) && DateTimeFormatterUtil.isValidDateTime(t2)) {
            return isAfterLatterDateTime(t1, t2, DateTimeFormatterUtil.DATETIME_FORMAT_PATTERN);
        } else {
    ...
    public static boolean isAfterOrEqualLatterDate(String t1, String t2) {
        if (DateTimeFormatterUtil.isValidTime(t1) && DateTimeFormatterUtil.isValidTime(t2)) {
            return isAfterOrEqualLatterDateTime(t1, t2, DateTimeFormatterUtil.TIME_FORMAT_PATTERN);
        } else if (DateTimeFormatterUtil.isValidDate(t1) && DateTimeFormatterUtil.isValidDate(t2)) {
            return isAfterOrEqualLatterDateTime(t1, t2, DateTimeFormatterUtil.DATE_FORMAT_PATTERN);
        } else if (DateTimeFormatterUtil.isValidDateTime(t1) && DateTimeFormatterUtil.isValidDateTime(t2)) {
            return isAfterOrEqualLatterDateTime(t1, t2, DateTimeFormatterUtil.DATETIME_FORMAT_PATTERN);
        } else {
    ...
    public static boolean isAfterLatterDateTime(String t1, String t2, String formatter) {
        if (DateTimeFormatterUtil.isDateTimeStringValidFormat(t1, formatter) && DateTimeFormatterUtil.isDateTimeStringValidFormat(t2, formatter)) {
            LocalDateTime time1 = DateTimeFormatterUtil.convertStringToDatetimeWithFomatter(t1, formatter);
            LocalDateTime time2 = DateTimeFormatterUtil.convertStringToDatetimeWithFomatter(t2, formatter);
    ...
    public static String calcTimeWithSecond(String time, int second) {
        try {
            LocalDateTime dateTime = DateTimeFormatterUtil.convertStringToDatetime(time);
            if (null != dateTime) {
                return DateTimeFormatterUtil.convertDatetimeToString(dateTime.plus(second, ChronoUnit.SECONDS));
            }
        } catch (Exception e) {
    ...
    public static String calcTimeWithMinute(String time, int min) {
        try {
            LocalDateTime dateTime = DateTimeFormatterUtil.convertStringToDatetime(time);
            if (null != dateTime) {
                return DateTimeFormatterUtil.convertDatetimeToString(dateTime.plus(min, ChronoUnit.MINUTES));
            }
        } catch (Exception e) {
            logger.error("Time string" + time + " format is illegal", e);
        }
    ...
    public static String calcTimeWithDay(String time, int day) {
        try {
            LocalDateTime dateTime = DateTimeFormatterUtil.convertStringToDatetime(time);
            if (null != dateTime) {
                return DateTimeFormatterUtil.convertDatetimeToString(dateTime.plus(day, ChronoUnit.DAYS));
            }
        } catch (Exception e) {
            logger.error("Time string" + time + " format is illegal", e);
        }
    ...
    public static List<String> getDateList(String start, String end) {
        LocalDate startDate = LocalDate.parse(start, DateTimeFormatterUtil.DATE_FORMATTER);
        LocalDate endDate = LocalDate.parse(end, DateTimeFormatterUtil.DATE_FORMATTER);
        List<String> dateList = new ArrayList<>();
        long period = ChronoUnit.DAYS.between(startDate, endDate) + 1;
        for (int i = 0; i < period; i++) {
            dateList.add(startDate.plusDays(i).format(DateTimeFormatterUtil.DATE_FORMATTER));
        }
        return dateList;
    }
    ...
    public static List<String> getDateList(LocalDate start, LocalDate end) {
        ...
        for (int i = 0; i < period; i++) {
            dateList.add(start.plusDays(i).format(DateTimeFormatterUtil.DATE_FORMATTER));
        }
        return dateList;
    }
