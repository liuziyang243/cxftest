...
import com.test.framework.util.collection.CollectionUtil;
import com.test.framework.util.reflect.ClassUtil;
import com.test.framework.util.text.StringUtil;
...
public class PropsUtil {
    ...
    public static Properties loadProps(String propsPath) {
        Properties props = loadOuterPorps(propsPath);
        if (props.isEmpty()) {
            return loadInnerProps(propsPath);
        }
        return props;
    }

    /**
     * 加载属性文件，属性文件位于打包中产生的resource文件夹中
     */
    public static Properties loadInnerProps(String propsPath) {
        ...
        try {
            ...
            if (is != null) {
                props.load(is);
                logger.info(propsPath + "从内部文件加载成功！");
            } else {
                logger.warn("内部资源文件" + propsPath + "不存在!");
            }
        } catch (Exception e) {
            logger.error(propsPath + "加载内部属性文件出错！", e);
            ...
            try {
                ...
                logger.error(propsPath + "释放内部属性资源出错！", e);
            }
        }
        return props;
    }
    ...
    /**
     * 加载运行jar包指令同目录下外部属性文件
     * 如果将java -jar指令封装成bat文件，则bat文件必须和jar包文件在同一个目录下
     *
     * @author lzy
     * @since 2016-09-23
     */
    public static Properties loadOuterPorps(String propsPath) {
        Properties props = new Properties();
        InputStream is = null;
        try {
            if (StringUtil.isEmpty(propsPath)) {
            ...
            if (is != null) {
                props.load(is);
                logger.info(propsPath + "从外部属性文件加载成功！");
            }
        } catch (Exception e) {
            logger.error(propsPath + "加载外部属性文件出错！继续加载内部资源文件！");
        } finally {
            try {
                if (is != null) {
                    is.close();
                ...
                logger.error(propsPath + "释放外部资源出错！", e);
            }
        }
        return props;
    }
